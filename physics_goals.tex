...
% It will allow the high statistics 
...
% and the INGRID proton module with $5\times 10^{20}$ POT.  
...
% and the INGRID proton module with 
$5\times 10^{20}$ POT. 
...
%neutrino interaction events at these two different location.
% Bottom two plots 
...
% Top two plots show the fluxes(left) and spectra of interaction events (right) for ND280 
...
% is the number of contaminated background events,
...
% is effectively estimated from side-band samples.
...
% So the interaction is largely affected by 
...
% in T2K and other interactions would bias the reconstructed energy.
...
% the gap between quasi-elastic region and pion-production region.% as shown in Fig. \ref{fig:electrono_scattering_data}.
...
% the precision of the event-by-event determination of the neutrino energy is not good.
% nor suffered from bias.
...
%low momentum protons in side acceptance.
%
%\begin{figure}[tbh]
%\begin{center}
...
% interactions of 600~MeV muon neutrinos on $^{12}C$ simulated with a mode.
...
% has a capability to distinguish the pion track and proton track from d$E$/d$x$, so 
